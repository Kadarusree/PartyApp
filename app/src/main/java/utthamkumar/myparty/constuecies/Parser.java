...
public class Parser {
    ...
    public String loadBooths() {
        String json = null;
        try {
            InputStream is = mActivity.getAssets().open("huzurngar");
            int size = is.available();
            byte[] buffer = new byte[size];
            is.read(buffer);
            is.close();
    ...
    public Booths getBooths(String constitucency) {
        ...
        try {
            ...
            for (int i = 0; i < boothsArray.length(); i++) {
                ...
                mBooth.setBoothNumber(boothObject.getString("Polling Booth No"));
                mBooth.setName(boothObject.getString("Polling Booth Name"));
                mBooth.setLocation(boothObject.getString("Polling Booth Location"));
